...
public class NinePuzzleq {  
  public static void main(String[] args) throws FileNotFoundException {
    ...
    setUpArray(current, finaal, play, filename,count);  
	
	//copy current[] to play[]
    /*for (int f = 0; f > 9;f++){
      play[f] = current[f];
	  f++;
	}*/
	...
	if (yn.equals("y") ||  yn.equals("Y")) {
	 // userInputs[0] = "";
      writeArrayToFile(current, finaal,play, filename);
	}
	//count = userInputs.length;
    while ( !solved && !save) {
	  System.out.println("hallo");	
      System.out.println("Current Puzzle");
      puzzle(play); // print current puzzle
      int lees = 0;
      ...
      if ( lees != 0 ) {
        if(lees == 99) {
          ...
          // move the index of the tiles
            if (move_index(lees, play)) {
              count++;
              solved = compare_solution(finaal, play);
             // userInputs[0] += Integer.toString(lees) + ",";
			 //play[0] += Integer.toString(play);
		      writeArrayToFile(current,finaal,play,filename);
		    }
          }  
  ...
  public static void setUpArray(int [] current, int [] finaal, int [] play,
	...
	//System.arraycopy( current, 0, play, 0, current.length );
	for(int g = 0; g < 9; g++){
		play[g]=current[g];
		g++;
	}
  ...
  public static boolean compare_solution(int [] finaal, int [] play) {
    for ( int i = 0; i < 9; i++) {
      if ( finaal[i] != play[i] ) {
        return false;
      }
